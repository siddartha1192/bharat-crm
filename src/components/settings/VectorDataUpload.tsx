...
export default function VectorDataUpload() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate file type - backend supports: .txt, .md, .csv, .json, .pdf, .xlsx, .xls, .docx, .doc
    const allowedTypes = ['.txt', '.md', '.csv', '.json', '.pdf', '.xlsx', '.xls', '.docx', '.doc'];
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload text files (.txt, .md), CSV, JSON, PDF, Excel (.xlsx, .xls), or Word (.docx, .doc) documents to enhance the AI knowledge base
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="border-2 border-dashed rounded-lg p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Supported formats: TXT, MD, CSV, JSON, PDF, Excel (XLSX, XLS), Word (DOCX, DOC)<br />
            ...
            <input
              ...
              type="file"
              className="hidden"
              accept=".txt,.md,.csv,.json,.pdf,.xlsx,.xls,.docx,.doc"
      ...
      {user?.role === 'ADMIN' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Advanced operations for managing the vector database
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Run Ingest Button */}
